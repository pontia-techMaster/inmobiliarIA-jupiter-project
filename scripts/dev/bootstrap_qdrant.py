...
def main() -> None:
    ...
    assert len(vectors[0]) == DIMENSIONS, f"expected {DIMENSIONS}-dim vectors, got {len(vectors[0])}"
    ...
    print(f"collection {collection!r} created (size={DIMENSIONS}, distance=COSINE)")
